...
class Request
{
    ...
    /**
     * Customizado por nathan para padrão de post
     */
    public static function postJson($url, $headers = array(), $body = null)
    {
        //print_r($body);

        return self::sendJson(Method::POST, $url, $body, $headers);
    }
     ...
     * @return Response
     */
    public static function sendJson($method, $url, $body = null, $headers = array())
    {
       /* self::$handle = curl_init();
        curl_setopt(self::$handle, CURLOPT_URL, self::encodeUrl($url));
        curl_setopt(self::$handle, CURLOPT_RETURNTRANSFER ,true);
        curl_setopt(self::$handle, CURLOPT_HEADER, FALSE);
		curl_setopt(self::$handle, CURLOPT_POST, true);
        curl_setopt(self::$handle, CURLOPT_POSTFIELDS, $body);
        curl_setopt(self::$handle,CURLOPT_HTTPHEADER ,self::getFormattedHeaders($headers));*/

      //  print_r($body);

        $body = '{"name":"Nathan Costa","email":"nathan.frc@hotmail.com","phone":"4738010919","mobilePhone":"4799376637","cpfCnpj":"65546324090","postalCode":"01310-000","address":"Av. Paulista","addressNumber":"150","complement":"Sala 201","province":"Centro","externalReference":"12987382","notificationDisabled":"","additionalEmails":"marcelo.almeida2@gmail.com,marcelo.almeida3@gmail.com","observations":"otimo pagador, nenhum problema ate o momento"}';
